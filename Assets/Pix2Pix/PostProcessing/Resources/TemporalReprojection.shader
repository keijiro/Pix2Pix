Shader "Hidden/Pix2Pix/TemporalReprojection"
{
    HLSLINCLUDE
    ...
    half4 FragComposite(VaryingsDefault i) : SV_Target
    {
        ...
        half4 prev = SAMPLE_TEX2D(_PrevTex, i.texcoord);
        half4 remap = SAMPLE_TEX2D(_UVRemap, i.texcoord);
        half4 c = SAMPLE_TEX2D(_MainTex, remap.xy);
        //c = half4(lerp(half3(1, 1, 1), c.rgb, remap.a), 1);
        //c = half4(lerp(SAMPLE_TEX2D(_MainTex,i.texcoord).rgb, c.rgb, remap.a), 1);
        //c = lerp(prev, c, 0.2 * remap.z);
        c = lerp(prev, lerp(SAMPLE_TEX2D(_MainTex,i.texcoord), c, remap.z), lerp(0.1, 0.5, remap.z));
        return c;
    }
